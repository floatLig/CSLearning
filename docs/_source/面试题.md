...
一个类是不可变的，是指一个对象被创建出来之后，在这个对象的生命周期，这个对象的属性状态是不会发生变化的。

String的底层 char[] value，char[] 被 private final修饰，同时的话，String这个类也是被final修饰，String没有提供set方法，修改字符串值的API，比如substring, 是创建一个新的String，String的方法跟属性就确保 char[] value的值不会发生变化。

正常的情况，value数组是不会发生变化的，但是我们仍然可以通过反射改变value的值。但是这样子是修改常量池里面的数据，比如说我们将“abc”修改为 “ddd”，我们以后创建出来的“abc”打印出来永远是“ddd”了。在这个进程就不存在“abc”

### String 不可变的好处

<https://floatlig.gitee.io/cs-notes/#/notes/Java%20%E5%9F%BA%E7%A1%80?id=%e4%b8%8d%e5%8f%af%e5%8f%98%e7%9a%84%e5%a5%bd%e5%a4%84>

1. String的hashcode也是经常被使用，比如HashMap中 String 当作 key,String不可变，hashcode也是不可变的，hashcode只需要计算一次。（hashcode的值存放在String对象中， [博客链接](https://blog.csdn.net/zxdfc/article/details/50154427)）
2. 线程安全，不可变天生就具有线程安全的特点
3. 可以使用常量池，一个对象不可变，我们以后创建相同的对象，只需要从常量池里面取得这个应用就可以了。
4. 安全性，String经常用作参数，如果说在网络连接中，String被变化，变化的一方以为现在连接的是其他主机，但是实际上并不是。

### 缓存池

Integer缓存池的大小是 -128 ~ 127。Integer缓存池的上界是可以调整的，在启动jvm的时候，我们通过 -XX:AutoBoxCacheMax=\<size\>来修改上界的大下

### 字符串常量池

字符串常量池 保存着所有字符串的字面量，这些字面量是在编译器就确定的。在运行过程中，也可以通过intern()方法向字符串常量池里面添加字面量。

当我们使用intern()方法的时候，如果说，字符串常量池里面有对应的字面量那么返回该字面量的值，否则，就在常量池里面添加字面量，并返回该引用。
...
toString()，不能是null对象的 toString()，否则会抛出异常。解决的方式可以为： 使用 valueOf()
...
- 属性：接口中只能默认是public static final修饰的常量，抽象类可以定义普通变量，默认为 `public static final`，不能修改为其他

- 构造器：他们都不能实例化，但是接口没有构造器，抽象类有构造器

- 方法：接口在JDK1.8之后能用default定义默认方法，抽象类没有限制。接口默认为`public default`，也可以使用`public default`，不能够使用 static, final
...
内部类在编译完之后，内部类和外部类是`两个不同的类`，只不过它们还是有联系的 （内部类持有外部类的引用）
...
TreeSet: 底层是红黑树，对添加进去的元素有排序，常用的方法：first(), last(), higer(), lower(); 遍历的结果是按照顺序的。

Queue： LinkedList（继承了Dequeu）, PriorityQueue(继承了AbstractQueue)
...
locks包： AQS， Lock， ReentrantLock, Condition


Executor 框架：Executors, FutureTask

BlockingQueue：ArrayBlockingQueue, LinkedBlockingQueue, PriorityBlockingQueue

ConcurrentHashMap, ConcurrentLinkedDequeue, CopyOnWriteArrayList, CountDownLatch, 
...
> MySQL的配置文件：<https://www.jianshu.com/p/5f39c486561>
> 字符集、大小写敏不敏感
> 默认的隔离级别
> 监听的端口3306， 最大连接数10000， 最长连接时间，积压
> 数据库文件所在的路径，数据所在的路径，临时文件所在的路径
...
这么做的原因是：innodb中页的默认大小是 16kb，如果说不存储数据的话，`使得数据更加集中`，对应的树可以更矮更胖，磁盘IO次数就会再次减少。（一般查找10亿数据，也只需要2次磁盘IO）
...
MySQL中视图，查询语句查询的中间结果，会用一个视图进行保存，是一个虚拟的表。

除此之外，还有一个 一致性视图（MVCC中的，低水位，undo log --> 实现 可重复读）
...
2. 手动杀死父进程，僵尸进程就变为孤儿进程，有init进程销毁
3. 调用父进程的 wait() waitpid(). wait() 父进程会阻塞，waitpid() 可以通过参数让父进程不阻塞
4. 两次fork()， 子进程退出，孙子进程成为孤儿进程，由init进程托管
