...
- 经典的垃圾回收器：Parallel Old。跟Parallel Scavenge配合，达到高吞吐量。标记整理算法

- 经典的垃圾回收器：CMS收集器

  - 目的是为了最短回收停顿时间
  - 步骤：初始标记  并发标记  最终标记(修正并发期间，一些标志的变动)  并发清除
  - 缺点：资源敏感（并发，占用CPU资源），浮动垃圾导致Concurrent Mode Failure，导致Full GC，碎片
...
> <https://cyc2018.github.io/CS-Notes/#/notes/HTTP?id=cookie>
>
> <https://juejin.im/post/5de4c3c76fb9a071b86cc482#heading-0>
>
> <https://floatlig.github.io/JavaLearning/#/_source/Browser-Server/Cookie-Session-servletContex>
...
- [Cookie 与 Session 选择]([https://cyc2018.github.io/CS-Notes/#/notes/HTTP?id=_1-%e7%94%a8%e9%80%94](https://cyc2018.github.io/CS-Notes/#/notes/HTTP?id=_1-用途))
...
| `获取`元素         | get name                    | (获取第一个元素) lrange list 0 0<br><br/>(通过下标) lindex list 0 | (随机获取)srandmember set 2               | hget hash one                                    |                                                              |
